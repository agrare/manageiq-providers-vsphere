  ...
  module HostSystem
    ...
    def parse_host_system_product(host_hash, props)
      if props.include?("summary.config.product.vendor")
        ...
        # TODO: vendor = "unknown" unless Host::VENDOR_TYPES.include?(vendor)
    ...
    def parse_host_system_operating_system(host, props)
      collections[:host_operating_systems].build(
        :host         => host,
    ...
    def parse_host_system_hardware(host, props)
      ...
      if props.include?("summary.hardware.memorySize")
        ...
        # TODO: hardware_hash[:memory_mb] = is_numeric?(memory_size) ? (memory_size.to_f / 1.megabyte).round : nil
        hardware_hash[:memory_mb] = memory_size.to_f / 1.megabyte
      end
      ...
      hardware = collections[:host_hardwares].build(hardware_hash)
    ...
    def parse_host_system_guest_devices(hardware, props)
      if props.include?("config.network.pnic")
        props["config.network.pnic"].to_a.each do |pnic|
          ...
          collections[:guest_devices].build(
            :hardware        => hardware,
            :uid_ems         => uid,
            :device_name     => name,
